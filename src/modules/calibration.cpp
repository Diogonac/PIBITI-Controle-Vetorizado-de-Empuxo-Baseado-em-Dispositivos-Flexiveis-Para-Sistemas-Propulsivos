...
void Calibration::calibra_servo_phi(void) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    printf("%f %f\r\n", mean_phi + offset_servo1,
           lista_angulos_1[i] + offset_servo1);
    wait(2);
  }
  ...
  for (int i = 0; i < 6; i++) {
    ...
    phi_data_calib[i] = mean_phi + offset_servo1;
    printf("%f %f\r\n", mean_phi + offset_servo1,
           lista_angulos_2[i] + offset_servo1);
    wait(2);
  }
...
void Calibration::calibra_servo_theta(void) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    printf("%f %f\r\n", mean_theta + offset_servo2,
           lista_angulos_1[i] + offset_servo2);
    wait(2);
  }
  ...
  for (int i = 0; i < 6; i++) {
    ...
    for (int y = 0; y < 500; y++) {
      ...
      // printf("%f %f\r\n", estimated_theta, lista_angulos_2[i] +
      // offset_servo2);
    }
    ...
    theta_data_calib[i] = mean_theta + offset_servo2;
    printf("%f %f\r\n", mean_theta + offset_servo2,
           lista_angulos_2[i] + offset_servo2);
    wait(2);
  }
...
void Calibration::angle_calib(double angle) {

  angle_calibrated = (P1 * angle) + P2;
}
...
void Calibration::phi_test_calib(void) {
  ...
  printf("\r\n");
  angle_calib(offset_servo1);
  servo1.position(angle_calibrated);
  angle_calib(offset_servo2);
  ...
  for (int i = 0; i < 6; i++) {
    angle_calib(lista_angulos_1[i] + offset_servo1);
    servo1.position(angle_calibrated);
    angle_calib(offset_servo2);
    servo2.position(angle_calibrated);
    wait(2);
    ...
    angle_calib(lista_angulos_1[i] + offset_servo1);
    printf("%f %f %f\r\n", mean_phi + offset_servo1,
           lista_angulos_1[i] + offset_servo1, angle_calibrated);
    wait(2);
  }
  ...
  for (int i = 0; i < 6; i++) {
    angle_calib(lista_angulos_2[i] + offset_servo1);
    servo1.position(angle_calibrated);
    angle_calib(offset_servo2);
    servo2.position(angle_calibrated);
    wait(2);
    ...
    angle_calib(lista_angulos_2[i] + offset_servo1);
    printf("%f %f %f\r\n", mean_phi + offset_servo1,
           lista_angulos_2[i] + offset_servo1, angle_calibrated);
    wait(2);
  }
...
void Calibration::config_calib_imu() {
  ...
  BNO055.set_mapping(4); // Ajuste do eixo de coordenadas / orientação P4 for
